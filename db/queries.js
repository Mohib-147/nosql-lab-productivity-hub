...
async function archiveProject(db, projectId) {
  return await db
    .collection('projects')
    .updateOne({ _id: projectId }, { $set: { archived: true } });
}
...
async function updateTaskStatus(db, taskId, newStatus) {
  return await db
    .collection('tasks')
    .updateOne({ _id: taskId }, { $set: { status: newStatus } });
}
...
async function addTaskTag(db, taskId, tag) {
  return await db
    .collection('tasks')
    .updateOne({ _id: taskId }, { $addToSet: { tags: tag } });
}
...
async function removeTaskTag(db, taskId, tag) {
  return await db
    .collection('tasks')
    .updateOne({ _id: taskId }, { $pull: { tags: tag } });
}
